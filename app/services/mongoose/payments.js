...
const getAllPayments = async (req) => {
  let condition = { laundryPartner: req.user.laundryPartner };
...
const createPayments = async (req) => {
  ...
  const check = await Payments.findOne({ type, laundryPartner: req.user.laundryPartner });
  ...
  const result = await Payments.create({
    image,
    type,
    laundryPartner: req.user.laundryPartner,
  });
...
const updatePayments = async (req) => {
  ...
  const result = await Payments.findOneAndUpdate(
    { _id: id },
    { type, image, laundryPartner: req.user.laundryPartner },
